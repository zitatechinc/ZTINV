...
class Command(BaseCommand):
    help = 'Import India and America countries, states, and cities safely (rerunnable)'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Filter countries to include only India (IN) and America (US)
        for c in countries_data:
            code = (c.get('iso2') or '').upper()
            # if code not in ['IN', 'US']:  # Only import India and USA
            if code not in ['IN']:  # Only import India 
                continue
        ...
        for s in states_data:
            ...
            # Skip states that do not belong to India or USA
            if country.code not in ['IN', 'US']:
                continue
        ...
        for c in cities_data:
            ...
            # Skip cities that do not belong to India or USA
            if country.code not in ['IN', 'US']:
                continue

...
class Command(BaseCommand):
    help = "Import India locations: State → Location, Telangana Cities → SubLocation"

    TELANGANA_STATE_NAME = "Telangana"
    ...
    def handle(self, *args, **kwargs):
        ...
        self.import_telangan_cities(cities_file, state_map)

        self.stdout.write(self.style.SUCCESS("✅ Telangana city import completed successfully"))
    ...
    # ===================== TELANGANA CITIES → SUBLOCATION =====================
    def import_telangan_cities(self, file_path, state_map):
        with open(file_path, encoding="utf-8") as f:
            data = json.load(f)
        ...
        for c in data:
            ...
            # ✅ ONLY TELANGANA
            if state.name.lower() != self.TELANGANA_STATE_NAME.lower():
                continue

            city_name = c["name"]

            # Same city + same state → skip
            ...
            # City exists in another state → rename
